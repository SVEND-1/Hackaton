...
import org.example.todolist.users.db.UserRepository;
...
public class UserService {
    ...
    public UserEntity getCurrentUserWithTask() {
        String email = SecurityContextHolder.getContext().getAuthentication().getName();
        UserEntity user =  userRepository.findByEmailEqualsIgnoreCase(email);
        notFoundUser(user);
        return user;
    ...
    public UserDTO findUserByEmailWithTasks(String email) {
        if (email == null) {
            log.debug("Пустой email поиск пользователя не возможен");
            throw new IllegalArgumentException("Пустой email пользователя");
        }

        UserEntity user = userRepository.findByEmailEqualsIgnoreCaseWithTasks(email);
